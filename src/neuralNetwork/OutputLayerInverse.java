...
public class OutputLayerInverse implements Layer
{
   ...
   */
   ...
   {
	  float temp;
	  for (int i = 0; i < expectedOutput.values.length; i++) {
		  exOut.values[i] = (1/1000)*expectedOutput.values[i];
	  }
	  float[] lossReturn = loss.derivative(exOut, output);
   ...
   /** Constructor. */
   public OutputLayerInverse(LossFunction loss, ActivationFunction func, WeightFiller fillerWeight, BiasFiller fillerBias, int in, int out, int executionPrevent)
   {
